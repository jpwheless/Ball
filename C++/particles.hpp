...
public:
	...
	//std::vector<std::int> ballVSortedX;
	//std::vector<z::Ball> ballVSortedY;
	...
	Particles(int *resXT, int *resYT, double *tickTimeT, double linGravityT) {
		...
		//ballVSortedX.reserve(MAX_PARTICLES);
		//ballVSortedY.reserve(MAX_PARTICLES);
		bhV.reserve(MAX_BH);
	...
	int sortParticlesX() {
		...
		for (int jGap = gSize - 1; jGap >= 0; --jGap) {		
			// Insertion sort with gap between sorted elements
			for (int currentGap = gaps[jGap]; currentGap < ballV.size(); ++currentGap) {
				
				// Store current ball
				z::Ball insBall = ballV[currentGap];
				
				// Search for correct place to insert current ball
				// Vector is sorted by the ball object's x bound value
				int j;
				for (j = currentGap - gaps[jGap];
						 j >= 0 && (ballV[j].x-ballV[j].radius) > (insBall.x-insBall.radius);
						 j -= gaps[jGap]) {
					// Swap values up the list to make room for current ball
					ballV[j + gaps[jGap]] = ballV[j];
				...
				// When appropriate place is found, insert current ball
				ballV[j + gaps[jGap]] = insBall;
			}
	...
	void addPhysics(unsigned int iStart, unsigned int iStop) {
		...
		for (int i = iStart; i < iStop; i++ ) {
			for (int j = i + 1; (ballV[j].x - ballV[j].radius) > (ballV[i].x + ballV[i].radius); j++) {
				collisionPhysics(i, j);
			}
		}
		
		for (int i = iStart; i < iStop; i++ ) {
			particleSingularPhysics(i);
			ballV[i].update();
		}
		if (iStart == 0) {
	...
	void addPhysics() {
		int ballVsize = ballV.size();
		int bhVsize = bhV.size();
		
		// Find collisions
		for (int i = 0; i < ballVsize; i++ ) {
			for (int j = i + 1; (ballV[j].x - ballV[j].radius) < (ballV[i].x + ballV[i].radius) && j < ballVsize; j++) {
				collisionPhysics(i, j);
			}
		}
	...
	void collisionPhysics(unsigned int i, unsigned int j) {
	...
	void particleSingularPhysics (unsigned int i) {
		int bhVsize = bhV.size();
